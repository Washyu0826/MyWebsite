...
function warnProfileUnavailable(message: string, error: unknown) {
...
export const getProfile = unstable_cache(async () => {
  ...
  if (profile.error || social.error) {
    warnProfileUnavailable('Unable to load profile; using fallback profile data.', profile.error || social.error);
    return { ...demoProfile, social_links: demoSocial };
  }
...
export const listExperiences = unstable_cache(async () => {
  ...
  if (error) {
    warnProfileUnavailable('Unable to load experiences; using fallback experience data.', error);
